...
<div class="page-content">
    ...
    <div class="card">
        <div class="card-body p-4">
            ...
            <hr />
            <form id="myForm" method='post' enctype="multipart/form-data" action="{{ route('update.product') }}">
                @csrf
    ...
    <div class="card">
        <div class="card-body">
            <table class="table mb-0 table-striped">
                ...
                <tbody>
                    <form method="post" action="{{ route('update.product.multi.image') }}"
                        enctype="multipart/form-data">
                        @csrf
                        @foreach($multiImgs as $key => $img)
                        <tr>
                            <th scope="col">{{ $key+1 }}</th>
                            <td><img src="{{ asset($img->photo_name) }}" style="width:60px; height:60px" alt=""></td>
                    ...
                    fRead.onload = (function(file) { //trigger function on successful read
                        return function(e) {
                            var img = $('<img/>').addClass('thumb').attr('src', e
                                .target.result).width(100).height(
        ...
        if (category_id) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var
                        d = $('select[name="subcategory_id" ]').empty();
